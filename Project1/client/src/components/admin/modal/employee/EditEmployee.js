...
import { UPDATE_EMPLOYEE } from '../../../../redux/constant';
   ...
   const dispatch = useDispatch();
    ...
    const [error, setError] = useState([]);
        ...
        // }
                ...
                useEffect(()=>{
           // if(state.students){
            const employee = allEmployees?.employees?.find(item => item?.id === id);
            ...
            if(employee){
                        setData({
                            ...
                            currentitem : employee.CurrentItem,
                            department : employee.Department,
                            ext : employee.Ext,
                            height : employee.Height,
                            weight : employee.Weight,
                            bloodtype : employee.BloodType,
                            rtelephone : employee.RTelephone,
                            ptelephone : employee.PTelephone,
                            rhouseno : employee.RHouseNo,
                            rhousestreet : employee.RHouseStreet,
                            rsubdivision : employee.RSubDivision,
                            phouseno : employee.PHouseNo,
                            phousestreet : employee.PHouseStreet,
                            psubdivision : employee.PSubDivision,
                            rbarangay : employee.RBarangay,
                            pbarangay : employee.PBarangay,
                            rzip : employee.RZip,
                            pzip : employee.PZip,
                            prefix : employee.Prefix,
                            suffix : employee.Suffix,
                            logincomputation : employee.LoginComputation   ,
                            role : 202
                            })

                      

                         console.log('ProfilePhoto:',employee.profilephoto);
                            
                  //  setData({ ...data , oldemail : student.email , oldusername: student.username })
        
                    setViewImg(`${process.env.REACT_APP_API_URL}/${employee.profilephoto}`)
                    
                    setPhoto()
                    setLoading(false)
                    }
          //  }
        },[id,allEmployees.employees])
    ...
    const hadleSubmit = async (e) => {
        ...
        console.log('dataall edit:',data.employmentstatus);
        ...
        if(response){
            if(response.status === 200){
                dispatch({ type : UPDATE_EMPLOYEE , payload : response.response.data })
                toast(response.response.message);
                setError('')
                props.esetOpen(false);
  ...
  return (
    ...
    <Dialog
        ...
        aria-describedby="alert-dialog-slide-description"
         ...
         maxWidth='lg'
    >
        <DialogTitle><h4>Edit Employee</h4></DialogTitle>
        <DialogContent>
